...
import vocento.devops.plugins.qualitygate.*;
...
public class SonarBreakMojo extends AbstractMojo {
    ...
    private void processResult(Result result) throws MojoExecutionException {
        ...
        switch (result.getStatus()) {
            case "ERROR":
                getLog().error("Se ha obtenido ERROR en la QualityGate");
                throw new MojoExecutionException("Build did not pass sonar tests.");
            case "WARNING":
                getLog().info("Build passed but warnings encountered.");
                break;
            case "OK":
